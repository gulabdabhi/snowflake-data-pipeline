...
from {{ ref('fact_sales_header') }} fh
join {{ ref('dim_date') }} d on fh.date_dim_key = d.date_dim_key
join {{ ref('dim_store') }} s on fh.store_dim_key = s.store_dim_key
join {{ ref('dim_country') }} cn on fh.country_dim_key = cn.country_dim_key
